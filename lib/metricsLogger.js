...
export async function logMetricsSnapshot({
  ...
  try {
    ...
    const snapshot = {
      ...
      // Dealer Gamma
      // Note: gamma.value is already in billions (e.g., -1.2 = -$1.2B)
      dealer_gamma_value: gamma.unavailable ? null : Math.abs(gamma.value || 0),
      dealer_gamma_direction: gamma.unavailable ? null : (gamma.sign || gamma.interpretation),
...
/**
 * Precompute and cache pattern analysis for a ticker
 * Call this daily to precompute patterns for faster /analyze responses
 * 
 * @param {string} ticker - Stock ticker symbol
 * @param {number} days - Number of days to retrieve (default: 30)
 * @returns {Promise<Object|null>} Metadata about available data for pattern analysis
 */
export async function precomputePatterns(ticker, days = 30) {
  if (!isMetricsLoggingEnabled()) {
    return null;
  }
  ...
  try {
    ...
    // Return metadata that can be cached
    return {
      ticker: ticker.toUpperCase(),
      ...
      hasGamma: history.some(h => h.dealer_gamma_value !== null),
      hasSkew: history.some(h => h.skew_value !== null),
      hasIV: history.some(h => h.atm_iv_value !== null),
      provenance: history.some(h => h.data_freshness === 'backfilled') ? 'mixed' : 'live'
    };
  } catch (err) {
    console.error(`[MetricsLogger] Error precomputing patterns for ${ticker}:`, err);
    return null;
  }
}
