...
export async function logMetricsSnapshot({
  ...
  try {
    ...
    const snapshot = {
      ...
      // ATM IV
      atm_iv_value: atmIV.unavailable ? null : atmIV.iv,
      atm_iv_strike: atmIV.unavailable ? null : atmIV.strike,
      
      // Put/Call Volume Ratio
      put_call_volume_ratio: putCallVolRatio.unavailable ? null : putCallVolRatio.ratio,
      
      // Implied Move
      implied_move_dollars: impliedMove.unavailable ? null : impliedMove.moveDollars,
      implied_move_pct: impliedMove.unavailable ? null : impliedMove.movePct,
      
      // Max Pain
      max_pain: maxPain.unavailable ? null : maxPain.strike,
      
      // Put/Call OI Ratio
      put_call_oi_ratio: putCallOIRatio.unavailable ? null : putCallOIRatio.ratio,
      
      // Total Delta
      total_delta_value: totalDelta.unavailable ? null : totalDelta.notional,
      
      // Gamma Walls (top 3)
      gamma_walls: gammaWalls.unavailable ? null : gammaWalls.walls.slice(0, 3).map(w => ({
        strike: w.strike,
        gamma_notional: w.gammaNotional
      })),
      
      // IV Term Structure
      iv_term_front: ivTerm.unavailable ? null : ivTerm.frontIV,
      iv_term_back: ivTerm.unavailable ? null : ivTerm.backIV,
      
      // Zero Gamma Level
      zero_gamma_level: zeroGammaLevel.unavailable ? null : zeroGammaLevel.level,
      
      // Multiple Expected Moves
      multiple_expected_moves: multipleExpectedMoves.unavailable ? null : 
        multipleExpectedMoves.moves.map(m => ({
          dte: m.dte,
          move_dollars: m.moveDollars,
          move_pct: m.movePct
        })),
      
      // Total Vega
      total_vega_value: totalVega.unavailable ? null : totalVega.notional,
      
      // Vanna
      vanna_value: vanna.unavailable ? null : vanna.notional,
