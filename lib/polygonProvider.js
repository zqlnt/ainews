...
export async function fetchPolygonOptions(symbol) {
  ...
  try {
    ...
    // Step 2: Get options contracts using the ticker endpoint (works with free tier)
    // Note: Polygon free tier may not include options. Use contract list endpoint instead
    const today = new Date().toISOString().split('T')[0];
    const optionsUrl = `${POLYGON_BASE_URL}/v3/reference/options/contracts?underlying_ticker=${symbol}&contract_type=call&limit=250&apiKey=${POLYGON_API_KEY}`;
    console.log(`[${new Date().toISOString()}] 🔍 Fetching options contracts for ${symbol}...`);
    
    const optionsResp = await fetch(optionsUrl, { timeout: 15000 });
...
function processPolygonOptions(results, spot) {
  ...
  for (const opt of results) {
    try {
      ...
      if (!greeks.implied_volatility || greeks.implied_volatility <= 0) continue;
      ...
      rows.push({
        ...
        iv: Math.round(greeks.implied_volatility * 10000) / 10000,
