...
export async function fetchPolygonOptions(symbol) {
  ...
  try {
    ...
    // Extract data timestamp from options snapshot (when the options data was last updated)
    // Options Greeks update every 15 minutes, so this reflects the actual data freshness
    // Look for last_updated in any contract's day object (nanosecond timestamp)
    ...
    for (const contract of allContracts) {
      if (contract.day?.last_updated) {
        ...
        break; // Use first available timestamp (all should be similar)
      }
    }
    ...
    console.log(`[${new Date().toISOString()}] 📅 Options data timestamp: ${dataTimestamp}`);
