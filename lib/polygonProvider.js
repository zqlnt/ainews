...
export async function fetchPolygonOptions(symbol) {
  ...
  try {
    ...
    if (maxLastUpdated > 0) {
      ...
      // Strategy:
      // - If last_updated is recent (< 2 hours): markets likely open, use fetch time - 15 min (15-min delayed)
      // - If last_updated is old (> 2 hours): markets closed, use last_updated (market close time)
      const hoursSinceUpdate = (now - timestampMs) / (1000 * 60 * 60);
      if (hoursSinceUpdate < 2) {
        // Recent - during market hours, data is 15-min delayed
        dataTimestamp = new Date(now - 15 * 60 * 1000).toISOString();
      } else {
        // Old - markets closed, use last_updated (market close time)
