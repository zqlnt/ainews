...
export async function fetchPolygonOptions(symbol) {
  ...
  try {
    ...
    // Extract data timestamp from Polygon response (when the price data is from)
    // Polygon returns timestamp in milliseconds
    const dataTimestampMs = priceData.results?.[0]?.t || null;
    const dataTimestamp = dataTimestampMs ? new Date(dataTimestampMs).toISOString() : new Date().toISOString();
    ...
    console.log(`[${new Date().toISOString()}] 📊 Got spot price for ${symbol}: $${spot} (data timestamp: ${dataTimestamp})`);
    ...
    const result = {
      ...
      dataTimestamp: dataTimestamp, // When the underlying data is from (market close time, etc.)
      atmIV: metrics.atmIV,
